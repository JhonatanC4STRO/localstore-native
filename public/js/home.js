// ════════════════════════════════════════
// ── Auto-detección de ciudad por geolocalización ──
// Solo corre si no hay una ciudad activa y el usuario no la rechazó antes.
// Cachea el resultado en sessionStorage para no volver a preguntar.
// ════════════════════════════════════════
(function autoDetectCity() {
  if (window.__cityFilterActive) return;            // Ya hay ciudad aplicada
  if (!('geolocation' in navigator)) return;        // Navegador sin geo
  // Si el usuario eligió "cambiar" para limpiar, no volvamos a auto-detectar en esa sesión
  ...
  // Si ya detectamos la ciudad antes en esta sesión, redirigir directo (sin volver a pedir permiso)
  ...
  navigator.geolocation.getCurrentPosition(
    (pos) => {
      ...
      const api = `https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=${lat}&lon=${lon}&zoom=10&addressdetails=1&accept-language=es`;
