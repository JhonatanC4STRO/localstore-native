...
switch ($method) {
    ...
    case 'POST':
        switch ($action) {
            ...
            case 'delete':     route_delete($conn, $body);              break;
...
function route_create(mysqli $db, array $body): void
{
    ...
    /* Privilege escalation guard: solo un Super Admin puede crear otro Super Admin */
    if ($role === 'super_admin' && !$isSuperAdmin) {
        json_error('Solo un Super Admin puede crear otros Super Admins', 403); return;
    }
...
function route_update(mysqli $db, array $body): void
{
    ...
    /* Cargar registro actual para verificar permisos */
    $cur = mysqli_prepare($db, "SELECT role FROM users WHERE id=? AND role IN ('admin','super_admin') AND deleted_at IS NULL LIMIT 1");
    mysqli_stmt_bind_param($cur, 'i', $id);
    mysqli_stmt_execute($cur);
    $target = mysqli_fetch_assoc(mysqli_stmt_get_result($cur));
    if (!$target) { json_error('Administrador no encontrado'); return; }

    /* Privilege escalation guard */
    if (!$isSuperAdmin) {
        if ($target['role'] === 'super_admin') { json_error('Solo un Super Admin puede editar a un Super Admin', 403); return; }
        if ($role === 'super_admin')           { json_error('Solo un Super Admin puede asignar el rol Super Admin', 403); return; }
    }

    /* Self-demotion guard */
    if ($id === $sessionId && $role !== 'super_admin' && $target['role'] === 'super_admin') {
...
/* ════════════════════════════════════
   POST — DELETE (soft)
════════════════════════════════════ */
function route_delete(mysqli $db, array $body): void
{
    global $isSuperAdmin, $sessionId;
    $id = (int)($body['id'] ?? 0);
    if ($id <= 0)           { json_error('ID inválido'); return; }
    if ($id === $sessionId) { json_error('No puedes eliminar tu propia cuenta'); return; }
    ...
    if ($target['role'] === 'super_admin' && !$isSuperAdmin) {
        json_error('Solo un Super Admin puede eliminar a un Super Admin', 403); return;
    }

    $stmt = mysqli_prepare($db,
        "UPDATE users SET deleted_at=NOW(), status='blocked', updated_at=NOW()
         WHERE id=? AND deleted_at IS NULL");
    mysqli_stmt_bind_param($stmt, 'i', $id);
    mysqli_stmt_execute($stmt);

    if (mysqli_stmt_affected_rows($stmt) > 0) {
        log_action($db, "Admin #{$id} eliminado");
        echo json_encode(['ok' => true, 'message' => 'Administrador eliminado']);
    } else {
        json_error('No se pudo eliminar el administrador');
    }
}
...
function log_action(mysqli $db, string $msg): void
{
    ...
    try {
        $stmt = mysqli_prepare($db, "INSERT INTO activity_log (admin_id, action, created_at) VALUES (?, ?, NOW())");
        if ($stmt) {
            mysqli_stmt_bind_param($stmt, 'is', $admin_id, $msg);
            mysqli_stmt_execute($stmt);
        }
