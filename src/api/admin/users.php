...
function route_list(mysqli $db): void
{
    ...
    $has_verified_col = mysqli_num_rows(mysqli_query($db, "SHOW COLUMNS FROM users LIKE 'verified_at'")) > 0;
    $has_seller_verif = mysqli_num_rows(mysqli_query($db, "SHOW TABLES LIKE 'seller_verifications'")) > 0;

    if ($verified === '1' || $verified === '0') {
        $conds = [];
        if ($has_verified_col) $conds[] = "u.verified_at IS NOT NULL";
        if ($has_seller_verif) $conds[] = "EXISTS (SELECT 1 FROM seller_verifications sv WHERE sv.user_id = u.id AND sv.status = 'approved')";
        if (!empty($conds)) {
            $expr = '(' . implode(' OR ', $conds) . ')';
            $where[] = $verified === '1' ? $expr : "NOT $expr";
        }
    }
    ...
    /* Build is_verified flag: verified if users.verified_at set OR has approved seller_verifications row */
    $has_verified = $has_verified_col;
    $parts = ['0'];
    if ($has_verified)     $parts[] = "(u.verified_at IS NOT NULL)";
    if ($has_seller_verif) $parts[] = "EXISTS (SELECT 1 FROM seller_verifications sv WHERE sv.user_id = u.id AND sv.status = 'approved')";
    $is_verified_expr = '(' . implode(' OR ', $parts) . ')';
    $verified_at_sel  = $has_verified ? "u.verified_at" : "NULL";
    $verified_sel     = "$is_verified_expr AS is_verified, $verified_at_sel AS verified_at,";
