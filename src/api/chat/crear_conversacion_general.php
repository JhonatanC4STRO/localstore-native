...
include_once(__DIR__ . '/../../config/user_settings.php');
...
if ($seller_id <= 0) {
    header("Location: ../../views/home.php?error=vendedor_no_encontrado");
    exit();
}

if ($buyer_id === $seller_id) {
    if ($product_id) {
        header("Location: ../../views/products/detalle.php?id=$product_id&msg=own_product");
    } else {
...
// Respetar preferencia de privacidad del vendedor
$chat_check = can_message_seller($conn, $buyer_id, $seller_id);
if (!$chat_check['ok']) {
    $reason = urlencode($chat_check['reason']);
    if ($product_id) {
        header("Location: ../../views/products/detalle.php?id=$product_id&msg=chat_blocked&reason=$reason");
    } else {
        header("Location: ../../views/seller_profile.php?seller_id=$seller_id&msg=chat_blocked&reason=$reason");
    }
    exit();
}
...
if ($res_check && mysqli_num_rows($res_check) > 0) {
    ...
    if (mysqli_query($conn, $ins_sql)) {
        ...
        header("Location: ../../views/home.php?error=chat_error");
        exit();
    }
}

header("Location: ../../views/chat/chat.php?conversation_id=$conversation_id");
exit();
