...
$sql = "SELECT
            ...
            (SELECT m2.message FROM messages m2 WHERE m2.conversation_id = c.id ORDER BY m2.id DESC LIMIT 1) AS last_message,
            (SELECT m3.created_at FROM messages m3 WHERE m3.conversation_id = c.id ORDER BY m3.id DESC LIMIT 1) AS last_message_at,
            IFNULL((SELECT COUNT(*) FROM messages m4 WHERE m4.conversation_id = c.id AND m4.sender_id != $uid AND m4.is_read = 0), 0) AS unread_count
...
<body>
  ...
  <div class="page-shell">
    ...
    <!-- ══ NAV SIDEBAR ══ -->
    <?php $activeTab = 'chat'; include __DIR__ . '/../../components/sidebar.php'; ?>
  ...
  <div class="app">
    ...
    <div class="conv-sidebar">
      ...
      <div class="conv-list" id="convList">
        <?php if (!empty($convList)): ?>
          <?php foreach ($convList as $idx => $c):
            ...
            $prodImg  = !empty($c['product_image']) ? '../../../public/uploads/products/' . $c['product_image'] : '';
            $lastMsg  = $c['last_message'] ?? '';
            $lastTime = '';
            if (!empty($c['last_message_at'])) {
                $diff = time() - strtotime($c['last_message_at']);
                if ($diff < 60)       $lastTime = 'Ahora';
                elseif ($diff < 3600) $lastTime = floor($diff / 60) . ' min';
                elseif ($diff < 86400) $lastTime = floor($diff / 3600) . ' h';
                else                   $lastTime = date('d/m', strtotime($c['last_message_at']));
            }
          ?>
            <div class="conv-item <?php echo ($c['id'] == $conversation_id) ? 'active' : ''; ?>"
              onclick="openChat(<?php echo $c['id']; ?>, '<?php echo addslashes($c['other_user'] ?? ''); ?>', '<?php echo addslashes($c['product_name'] ?? ''); ?>', '<?php echo $initial; ?>', <?php echo $isOnline ? 'true' : 'false'; ?>, '<?php echo addslashes($price ?? ''); ?>', '<?php echo addslashes($prodImg ?? ''); ?>', <?php echo intval($c['product_id'] ?? 0); ?>, <?php echo ((int)$c['is_seller']) ? 'true' : 'false'; ?>, '<?php echo addslashes($c['product_status'] ?? ''); ?>')"
              ...
              <div class="ci-body">
                <div class="ci-top">
                  ...
                  <div class="ci-time"><?php echo htmlspecialchars($lastTime); ?></div>
                </div>
                <div class="ci-bottom">
                  <div class="ci-preview"><?php echo htmlspecialchars($lastMsg ?: 'Sin mensajes aún'); ?></div>
                  <?php if ($unread > 0): ?>
      ...
      <div style="padding:12px 14px;border-top:1.5px solid var(--border);display:flex;align-items:center;gap:10px;background:var(--g50);">
        ...
        <a href="./home.php" style="color:var(--ink3);font-size:.95rem;transition:color .2s;" title="Ir al inicio"><i class="bi bi-house-fill"></i></a>
      </div>
      ...
      <div id="chatActive" style="display:none;flex-direction:column;height:100%;">
        ...
        <div class="chat-header">
          ...
          <div class="ch-actions">
            <button type="button" id="chSoldBtn" class="ch-sold-pill is-mark" style="display:none;"
                    onclick="toggleConvProductSold()">
              <i class="bi bi-check2-circle" id="chSoldBtnIcon"></i>
              <span id="chSoldBtnLabel">Marcar vendido</span>
            </button>
            <span id="chSoldChip" class="ch-sold-chip" style="display:none;">
              <i class="bi bi-check-circle-fill"></i> Vendido
            </span>
        ...
        <div class="product-pin" id="productPin">
          ...
          <a href="../products/detalle.php?id=<?php echo $c['product_id']; ?>" id="ppVerBtn" class="pp-btn">
